...
enum instruction {FW_1,TURN_90,STOP};

instruction state = FW_1;



void forward(double distance){//distance in meter
  ...
  if(afgelegdeWegTicks < abs(distance)*9180){
    ...
    if (currentMillis - previousMillisForward >= 50) {// per 10ms
...
void calculateSpeed(){
  ...
  if (currentMillis - previousMillis >= 50) {// per 10ms
